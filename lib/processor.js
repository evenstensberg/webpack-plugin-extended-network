...
module.exports = class Processor {
  ...
  on(eventName, cb) {
    return this._eventEmitter.on(eventName, cb);
  }

  once(eventName, cb) {
    return this._eventEmitter.once(eventName, cb);
  }
  ...
  getScriptTags() {
    return this.sendCommand('DOM.enable')
      .then(_ => this.sendCommand('DOM.getFlattenedDocument'))
      .then(res => res.nodes.forEach( (node) => {
        if (node.attributes) {
          node.attributes.forEach( (attri) => {
            if (this._chunkNames.includes(attri)) {
              this._chunkObjects.forEach( (chunkObj) => {
                if (chunkObj.chunkName === attri) {
                  chunkObj.injectionType = node.localName;
                  chunkObj.injectionChildren = node.children;
                  chunkObj.attributes = node.attributes;
                }
              });
            }
          });
